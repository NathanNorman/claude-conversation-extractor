...
class ClaudeConversationExtractor {
  ...
  async findConversations(): Promise<ConversationFile[]> {
  ...
  async searchConversations(query: string, conversations: ConversationFile[]): Promise<SearchResult[]> {
    const results: SearchResult[] = [];
    const queryLower = query.toLowerCase();
    
    for (const conversation of conversations) {
      try {
        const content = await fs.readFile(conversation.path, 'utf-8');
  ...
  async displayConversation(conversation: ConversationFile): Promise<void> {
    console.clear();
    ...
    try {
      ...
      for (const line of lines.slice(0, 50)) { // Show first 50 messages
        try {
          const parsed = JSON.parse(line);
      ...
      if (lines.length > 50) {
        console.log(chalk.gray(`... and ${lines.length - 50} more messages`));
      }
...
async function showSearchInterface(): Promise<void> {
  ...
  // Format results for selection menu
  const choices = results.slice(0, 15).map((result, index) => {
    ...
    const preview = result.matches[0]?.slice(0, 60) || '';
    
    return {
      name: `${chalk.cyan(date)} | ${chalk.magenta(project)} | ${chalk.green(relevance + '%')} | ${chalk.gray(preview + '...')}`,
      value: result.file,
      short: `${project} (${date})`
    };
  });
  ...
  if (results.length > 15) {
    choices.push({
      name: chalk.gray(`... and ${results.length - 15} more results`),
      value: null,
      short: 'More results available'
    });
  }
  ...
  const { selectedConversation } = await inquirer.prompt([
    {
      ...
      message: `Select conversation (${results.length} found):`,
      choices,
      pageSize: 10,
      loop: false
    }
  ...
  if (selectedConversation) {
    ...
    const { action } = await inquirer.prompt([
      {
        ...
        choices: [
          { name: '📋 Copy filename to clipboard', value: 'copy' },
    ...
    switch (action) {
      case 'copy':
        console.log(chalk.green(`📋 Filename: ${selectedConversation.path}`));
        break;
      case 'edit':
        console.log(chalk.blue(`📝 Opening ${selectedConversation.name} in editor...`));
        // Could add actual editor opening here
        break;
      case 'export':
        console.log(chalk.green(`📄 Exporting ${selectedConversation.name}...`));
        // Could add actual export here
        break;
      case 'back':
